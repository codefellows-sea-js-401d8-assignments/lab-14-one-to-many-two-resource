...
partyRouter.get('/', (req, res, next) => {
  let handleDbError = res.sendError(AppError.error500('err 500').respond(res), next);
  Party.find().then(res.json.bind(res), handleDbError);
});

partyRouter.get('/:id', (req, res, next) => {
  let handleDbError = res.sendError(AppError.error400('err 400').respond(res));
  let fourOhFour = res.sendError(AppError.error404('err 404').respond(res));
...
partyRouter.post('/', jsonParser, (req, res, next) => {
  let handleBadValidation = res.sendError(AppError.error400('err 400').respond(res), next);
  (Party(req.body)).save().then(res.json.bind(res), handleBadValidation);
});

partyRouter.put('/:id', jsonParser, (req, res, next) => {
  (Party.findOneAndUpdate({'_id': req.params.id}, req.body).then(res.json.bind(res), res.sendError(AppError.error400('err 400').respond(res)), next));
});

partyRouter.delete('/:id', (req, res, next) => {
  (Party.remove({'_id': req.params.id})).then(res.json.bind(res), res.sendError(AppError.error400('err 400').respond(res)), next);
});
